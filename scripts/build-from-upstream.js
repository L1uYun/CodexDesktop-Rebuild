...
function getRebuildAvatarAutoOpenRuntimeSnippet() {
  return [
    "(()=>{if(globalThis.__codexRebuildAvatarAutoOpenVersion!==5){globalThis.__codexRebuildAvatarAutoOpenVersion=5;try{",
    "let e={x:0,y:0,heading:Math.random()*6.283185307179586,speed:20,last:Date.now(),near:0,sector:0};let t=(e,t)=>e.x<=t.x&&e.x<t.x+t.width&&e.y>=t.y&&e.y<t.y+t.height,q=e=>Math.max(0,Math.min(7,Math.round((((e%360)+360)%360)/45)%8));let r=()=>{try{let r=M.avatarOverlayManager.window;if(!r||r.isDestroyed())return;let i=r.getBounds(),a=n.screen.getAllDisplays();if(!a.length)return;let o={x:i.x+i.width/2,y:i.y+i.height/2},s=n.screen.getCursorScreenPoint(),c=a.find(e=>t(o,e.bounds))||n.screen.getDisplayNearestPoint(o),l=a.find(e=>t(s,e.bounds))||n.screen.getDisplayNearestPoint(s),u=l.workArea,d=l.id!==c.id?{x:Math.min(c.workArea.x,l.workArea.x),y:Math.min(c.workArea.y,l.workArea.y),width:Math.max(c.workArea.x+c.workArea.width,l.workArea.x+l.workArea.width)-Math.min(c.workArea.x,l.workArea.x),height:Math.max(c.workArea.y+c.workArea.height,l.workArea.y+l.workArea.height)-Math.min(c.workArea.y,l.workArea.y)}:u,p=Date.now(),m=Math.max(.025,Math.min(.12,(p-e.last)/1e3));e.last=p;Math.abs((e.x||i.x)-i.x)>24||Math.abs((e.y||i.y)-i.y)>24?(e.x=i.x,e.y=i.y,e.heading=Math.random()*6.283185307179586,e.speed=18+Math.random()*10):(e.heading+=(Math.random()-.5)*.84*m,e.speed+=(18+Math.random()*12-e.speed)*Math.min(1,m*.35));let h=Math.cos(e.heading)*e.speed,g=Math.sin(e.heading)*e.speed*.72,_=0,v=0,b=120,x=d.x,S=d.y,C=Math.max(x,d.x+d.width-i.width),w=Math.max(S,d.y+d.height-i.height);e.x<x+b&&(_+=(x+b-e.x)/b),e.x>C-b&&(_-=(e.x-(C-b))/b),e.y<S+b&&(v+=(S+b-e.y)/b),e.y>w-b&&(v-=(e.y-(w-b))/b);if(l.id!==c.id){let t=l.workArea,rx=Math.min(Math.max(t.x,e.x),Math.max(t.x,t.x+t.width-i.width))-e.x,ry=Math.min(Math.max(t.y,e.y),Math.max(t.y,t.y+t.height-i.height))-e.y;h+=Math.max(-18,Math.min(18,rx/42)),g+=Math.max(-10,Math.min(10,ry/70)),Math.abs(rx)>8&&(e.heading=Math.atan2(g/.72,h))}let T=e.x+i.width/2,E=e.y+i.height/2,D=T-s.x,O=E-s.y,k=Math.hypot(D,O);if(k>=150)e.near=0;else if(!e.near)e.near=p;else if(k>.001&&p-e.near>=900){let t=((150-k)/150)**2*2.1;h+=D/k*t*32,g+=O/k*t*32}h+=_*38,g+=v*30;(Math.abs(_)>0.4||Math.abs(v)>0.4)&&(e.heading=Math.atan2(g/.72,h));e.x+=h*m,e.y+=g*m;let A=e.x<x||e.x>C,j=e.y<S||e.y>w;e.x=Math.max(x,Math.min(C,e.x)),e.y=Math.max(S,Math.min(w,e.y)),A&&(e.heading=Math.PI-e.heading),j&&(e.heading=-e.heading);let P=Math.round(e.x),R=Math.round(e.y),I=q(Math.atan2(g,h)*180/Math.PI);I!==e.sector&&(e.sector=I,r.webContents.executeJavaScript(`window.__codexAvatarWalkDirection=${I}`).catch(()=>{}));(Math.abs(P-i.x)>=1||Math.abs(R-i.y)>=1)&&M.avatarOverlayManager.autoMove(r.webContents.id,P,R)}catch(e){globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-error`,e&&e.stack||String(e))}};clearInterval(globalThis.__codexRebuildAvatarWalkTimer),globalThis.__codexRebuildAvatarWalkTimer=setInterval(r,45),globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-started`);",
