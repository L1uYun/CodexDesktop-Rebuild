...
function getRebuildAvatarAutoOpenRuntimeSnippet() {
  return [
    ...
    "let e={x:0,y:0,vx:0,vy:0,last:Date.now(),near:0,sector:0,home:0,from:0};let t=(e,t)=>e.x>=t.x&&e.x<t.x+t.width&&e.y>=t.y&&e.y<t.y+t.height,q=e=>Math.max(0,Math.min(7,Math.round((((e%360)+360)%360)/45)%8)),r=(e,t,r)=>Math.max(e,Math.min(t,r));let i=()=>{try{let i=M.avatarOverlayManager.window;if(!i||i.isDestroyed())return;let a=i.getBounds(),o=M.avatarOverlayManager.getLayout(i).mascot,s={x:a.x+o.left,y:a.y+o.top,width:o.width,height:o.height},c=n.screen.getAllDisplays();if(!c.length)return;let l={x:s.x+s.width/2,y:s.y+s.height/2},u=n.screen.getCursorScreenPoint(),d=c.find(e=>t(l,e.bounds))||n.screen.getDisplayNearestPoint(l),p=c.find(e=>t(u,e.bounds))||n.screen.getDisplayNearestPoint(u),f=p.id!==d.id;if(f)e.home=p.id,e.from=d.id;else if(e.home===p.id){let t=p.workArea;f=!(s.x>=t.x+24&&s.y>=t.y+24&&s.x+s.width<=t.x+t.width-24&&s.y+s.height<=t.y+t.height-24),f||(e.home=0,e.from=0)}else e.home=0,e.from=0;let y=f&&e.from?(c.find(t=>t.id===e.from)||d):d,m=f?{x:Math.min(y.workArea.x,p.workArea.x),y:Math.min(y.workArea.y,p.workArea.y),width:Math.max(y.workArea.x+y.workArea.width,p.workArea.x+p.workArea.width)-Math.min(y.workArea.x,p.workArea.x),height:Math.max(y.workArea.y+y.workArea.height,p.workArea.y+p.workArea.height)-Math.min(y.workArea.y,p.workArea.y)}:d.workArea,h=Date.now(),g=Math.max(.025,Math.min(.12,(h-e.last)/1e3));e.last=h;Math.abs((e.x||s.x)-s.x)>24||Math.abs((e.y||s.y)-s.y)>24?(e.x=s.x,e.y=s.y,e.vx=0,e.vy=0):0;let _=m.x,v=m.y,b=Math.max(_,m.x+m.width-s.width),x=Math.max(v,m.y+m.height-s.height),S=150,C=0,w=0;e.x<_+S&&(C+=((_+S-e.x)/S)*70),e.x>b-S&&(C-=((e.x-(b-S))/S)*70),e.y<v+S&&(w+=((v+S-e.y)/S)*52),e.y>x-S&&(w-=((e.y-(x-S))/S)*52);if(f){let t=p.workArea,n=Math.min(Math.max(t.x,e.x),Math.max(t.x,t.x+t.width-s.width))-e.x,r=Math.min(Math.max(t.y,e.y),Math.max(t.y,t.y+t.height-s.height))-e.y,i=Math.hypot(n,r);i>.001&&(C=n/i*360,w=r/i*220,e.vx=n/i*115,e.vy=r/i*70)}let T=e.x+s.width/2,E=e.y+s.height/2,D=T-u.x,O=E-u.y,k=Math.hypot(D,O);if(f||k>=70)e.near=0;else if(!e.near)e.near=h;else if(k>.001&&h-e.near>=900){let t=((70-k)/70)**2*1.5;C+=D/k*t*22,w+=O/k*t*22}!f&&(C+=(Math.random()-.5)*34,w+=(Math.random()-.5)*26),e.vx+=C*g,e.vy+=w*g,e.vx*=Math.exp(-.75*g),e.vy*=Math.exp(-.75*g);let A=Math.hypot(e.vx,e.vy),j=f?130:38;if(A<12){let t=Math.random()*6.283185307179586;e.vx+=Math.cos(t)*8,e.vy+=Math.sin(t)*6}else if(A>j){e.vx=e.vx/A*j,e.vy=e.vy/A*j}e.x+=e.vx*g,e.y+=e.vy*g;let P=e.x,R=e.y;e.x=r(_,b,e.x),e.y=r(v,x,e.y),e.x!==P&&(e.vx=Math.abs(e.vx)*(P<_?1:-1)),e.y!==R&&(e.vy=Math.abs(e.vy)*(R<v?1:-1));let I=Math.round(e.x-o.left),L=Math.round(e.y-o.top),B=q(Math.atan2(e.vy,e.vx)*180/Math.PI);B!==e.sector&&(e.sector=B,i.webContents.executeJavaScript(`window.__codexAvatarWalkDirection=${B}`).catch(()=>{}));(Math.abs(I-a.x)>=1||Math.abs(L-a.y)>=1)&&M.avatarOverlayManager.autoMove(i.webContents.id,I,L)}catch(e){globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-error`,e&&e.stack||String(e))}};clearInterval(globalThis.__codexRebuildAvatarWalkTimer),globalThis.__codexRebuildAvatarWalkTimer=setInterval(i,45),globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-started`);",
