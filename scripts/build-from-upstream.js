...
function getRebuildAvatarAutoOpenRuntimeSnippet() {
  return [
    ...
    "let e={x:0,y:0,vx:0,vy:0,last:Date.now(),near:0,sector:0,home:0,from:0,target:0,mode:`rest`,sentMode:null,until:0,heading:0,speed:0};let t=(e,t)=>e.x>=t.x&&e.x<t.x+t.width&&e.y>=t.y&&e.y<t.y+t.height,q=e=>Math.max(0,Math.min(7,Math.round((((e%360)+360)%360)/45)%8)),r=(e,t,r)=>Math.max(e,Math.min(t,r));let i=()=>{try{let i=M.avatarOverlayManager.window;if(!i||i.isDestroyed())return;let a=i.getBounds(),o=M.avatarOverlayManager.getLayout(i).mascot,s={x:a.x+o.left,y:a.y+o.top,width:o.width,height:o.height},c=n.screen.getAllDisplays();if(!c.length)return;let l={x:s.x+s.width/2,y:s.y+s.height/2},u=n.screen.getCursorScreenPoint(),d=c.find(e=>t(l,e.bounds))||n.screen.getDisplayNearestPoint(l),p=c.find(e=>t(u,e.bounds))||n.screen.getDisplayNearestPoint(u);e.target!==p.id&&(e.target=p.id);let f=e.target!==d.id;if(f)e.home=e.target,e.from||(e.from=d.id);let z=c.find(t=>t.id===e.target)||p;if(!f&&e.home===e.target){let t=z.workArea;f=!(s.x>=t.x+24&&s.y>=t.y+24&&s.x+s.width<=t.x+t.width-24&&s.y+s.height<=t.y+t.height-24),f||(e.home=0,e.from=0)}else f||(e.home=0,e.from=0);let y=f&&e.from?(c.find(t=>t.id===e.from)||d):d,m=f?{x:Math.min(y.workArea.x,z.workArea.x),y:Math.min(y.workArea.y,z.workArea.y),width:Math.max(y.workArea.x+y.workArea.width,z.workArea.x+z.workArea.width)-Math.min(y.workArea.x,z.workArea.x),height:Math.max(y.workArea.y+y.workArea.height,z.workArea.y+z.workArea.height)-Math.min(y.workArea.y,z.workArea.y)}:d.workArea,h=Date.now(),g=Math.max(.025,Math.min(.12,(h-e.last)/1e3));e.last=h;Math.abs((e.x||s.x)-s.x)>24||Math.abs((e.y||s.y)-s.y)>24?(e.x=s.x,e.y=s.y,e.vx=0,e.vy=0,e.mode=`probe`,e.until=0,e.from=d.id):0;let _=m.x,v=m.y,b=Math.max(_,m.x+m.width-s.width),x=Math.max(v,m.y+m.height-s.height),T=e.x+s.width/2,E=e.y+s.height/2,D=T-u.x,O=E-u.y,k=Math.hypot(D,O),C=0,w=0;if(f||k>=118)e.near=0;else if(!e.near)e.near=h;if(f){let t=z.workArea,n=Math.min(Math.max(t.x,e.x),Math.max(t.x,t.x+t.width-s.width))-e.x,r=Math.min(Math.max(t.y,e.y),Math.max(t.y,t.y+t.height-s.height))-e.y,i=Math.hypot(n,r);e.mode=`home`,e.until=h+900,i>.001?(C=n/i*46,w=r/i*32,e.heading=Math.atan2(r,n)):0}else if(k>.001&&e.near&&h-e.near>=420&&k<118){e.mode=`retreat`,e.until=h+650,C=D/k*42,w=O/k*32,e.heading=Math.atan2(O,D)}else{if(h>=e.until){let t=Math.random();t<.34?(e.mode=`rest`,e.until=h+800+Math.random()*1600,e.speed=0):t<.58?(e.mode=`probe`,e.until=h+450+Math.random()*900,e.heading+=Math.random()*1.1-.55,e.speed=0):(e.mode=`creep`,e.until=h+1200+Math.random()*1800,e.heading=e.heading+(Math.random()-.5)*1.5,e.speed=14+Math.random()*13)}e.mode===`creep`?(C=Math.cos(e.heading)*e.speed,w=Math.sin(e.heading)*e.speed):e.mode===`probe`?(e.heading+=Math.sin(h/210)*.035,C=0,w=0):(C=0,w=0)}let A=.24;e.vx+=(C-e.vx)*A,e.vy+=(w-e.vy)*A,e.mode===`rest`&&(e.vx*=Math.exp(-3.2*g),e.vy*=Math.exp(-3.2*g)),e.mode===`probe`&&(e.vx*=Math.exp(-2.6*g),e.vy*=Math.exp(-2.6*g));let j=Math.hypot(e.vx,e.vy),P=e.mode===`home`?46:e.mode===`retreat`?42:27;j>P&&(e.vx=e.vx/j*P,e.vy=e.vy/j*P),e.x+=e.vx*g,e.y+=e.vy*g;let R=e.x,N=e.y;e.x=r(_,b,e.x),e.y=r(v,x,e.y),e.x!==R&&(e.vx=0,e.heading=Math.PI-e.heading,e.mode=`probe`,e.until=h+650),e.y!==N&&(e.vy=0,e.heading=-e.heading,e.mode=`probe`,e.until=h+650);let I=Math.round(e.x-o.left),L=Math.round(e.y-o.top),B=q((j>.7?Math.atan2(e.vy,e.vx):e.heading)*180/Math.PI);(B!==e.sector||e.sentMode!==e.mode)&&(e.sector=B,e.sentMode=e.mode,i.webContents.executeJavaScript(`window.__codexAvatarWalkDirection=${B};window.__codexAvatarMotionMode=${JSON.stringify(e.mode)}`).catch(()=>{}));(Math.abs(I-a.x)>=1||Math.abs(L-a.y)>=1)&&M.avatarOverlayManager.autoMove(i.webContents.id,I,L)}catch(e){globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-error`,e&&e.stack||String(e))}};clearInterval(globalThis.__codexRebuildAvatarWalkTimer),globalThis.__codexRebuildAvatarWalkTimer=setInterval(i,45),globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-started`);",
