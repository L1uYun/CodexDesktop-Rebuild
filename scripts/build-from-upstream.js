...
function getRebuildAvatarAutoOpenRuntimeSnippet() {
  return [
    "(()=>{if(globalThis.__codexRebuildAvatarAutoOpenVersion!==7){globalThis.__codexRebuildAvatarAutoOpenVersion=7;try{",
    "let e={x:0,y:0,heading:Math.random()*6.283185307179586,speed:20,last:Date.now(),near:0,sector:0};let t=(e,t)=>e.x<=t.x&&e.x<t.x+t.width&&e.y>=t.y&&e.y<t.y+t.height,q=e=>Math.max(0,Math.min(7,Math.round((((e%360)+360)%360)/45)%8));let r=()=>{try{let r=M.avatarOverlayManager.window;if(!r||r.isDestroyed())return;let i=r.getBounds(),a=M.avatarOverlayManager.getLayout(r).mascot,o={x:i.x+a.left,y:i.y+a.top,width:a.width,height:a.height},s=n.screen.getAllDisplays();if(!s.length)return;let c={x:o.x+o.width/2,y:o.y+o.height/2},l=n.screen.getCursorScreenPoint(),u=s.find(e=>t(c,e.bounds))||n.screen.getDisplayNearestPoint(c),d=s.find(e=>t(l,e.bounds))||n.screen.getDisplayNearestPoint(l),p=d.workArea,m=d.id!==u.id?{x:Math.min(u.workArea.x,d.workArea.x),y:Math.min(u.workArea.y,d.workArea.y),width:Math.max(u.workArea.x+u.workArea.width,d.workArea.x+d.workArea.width)-Math.min(u.workArea.x,d.workArea.x),height:Math.max(u.workArea.y+u.workArea.height,d.workArea.y+d.workArea.height)-Math.min(u.workArea.y,d.workArea.y)}:p,h=Date.now(),g=Math.max(.025,Math.min(.12,(h-e.last)/1e3));e.last=h;Math.abs((e.x||o.x)-o.x)>24||Math.abs((e.y||o.y)-o.y)>24?(e.x=o.x,e.y=o.y,e.heading=Math.random()*6.283185307179586,e.speed=18+Math.random()*10):(e.heading+=(Math.random()-.5)*.84*g,e.speed+=(18+Math.random()*12-e.speed)*Math.min(1,g*.35));let _=Math.cos(e.heading)*e.speed,v=Math.sin(e.heading)*e.speed*.72,b=0,x=0,S=120,C=m.x,w=m.y,T=Math.max(C,m.x+m.width-o.width),E=Math.max(w,m.y+m.height-o.height);e.x<C+S&&(b+=(C+S-e.x)/S),e.x>T-S&&(b-=(e.x-(T-S))/S),e.y<w+S&&(x+=(w+S-e.y)/S),e.y>E-S&&(x-=(e.y-(E-S))/S);if(d.id!==u.id){let t=d.workArea,rx=Math.min(Math.max(t.x,e.x),Math.max(t.x,t.x+t.width-o.width))-e.x,ry=Math.min(Math.max(t.y,e.y),Math.max(t.y,t.y+t.height-o.height))-e.y;_=Math.max(-80,Math.min(80,rx/10)),v=Math.max(-42,Math.min(42,ry/18)),e.heading=Math.atan2(v/.72,_)}let D=e.x+o.width/2,O=e.y+o.height/2,k=D-l.x,A=O-l.y,j=Math.hypot(k,A);if(u.id!==d.id||j>=70)e.near=0;else if(!e.near)e.near=h;else if(j>.001&&h-e.near>=900){let t=((70-j)/70)**2*1.5;_+=k/j*t*22,v+=A/j*t*22}_+=b*38,v+=x*30;(Math.abs(b)>0.4||Math.abs(x)>0.4)&&(e.heading=Math.atan2(v/.72,_));e.x+=_*g,e.y+=v*g;let P=e.x<C||e.x>T,R=e.y<w||e.y>E;e.x=Math.max(C,Math.min(T,e.x)),e.y=Math.max(w,Math.min(E,e.y)),P&&(e.heading=Math.PI-e.heading),R&&(e.heading=-e.heading);let I=Math.round(e.x-a.left),L=Math.round(e.y-a.top),B=q(Math.atan2(v,_)*180/Math.PI);B!==e.sector&&(e.sector=B,r.webContents.executeJavaScript(`window.__codexAvatarWalkDirection=${B}`).catch(()=>{}));(Math.abs(I-i.x)>=1||Math.abs(L-i.y)>=1)&&M.avatarOverlayManager.autoMove(r.webContents.id,I,L)}catch(e){globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-error`,e&&e.stack||String(e))}};clearInterval(globalThis.__codexRebuildAvatarWalkTimer),globalThis.__codexRebuildAvatarWalkTimer=setInterval(r,45),globalThis.__codexRebuildTrace&&globalThis.__codexRebuildTrace(`avatar-main-walk-started`);",
