...
from codex_session_delete.rebuild_config import APP_EXECUTABLE_NAMES, DEFAULT_APP_DIR
...
def build_codex_executable(app_dir: Path) -> Path:
    ...
    candidates = [app_dir / name for name in APP_EXECUTABLE_NAMES]
...
def running_windows_codex_process_id(app_dir: Path) -> int | None:
    if sys.platform != "win32":
        return None
    expected_root = str(app_dir).lower().replace("/", "\\")
    script = (
        "Get-CimInstance Win32_Process -Filter \"Name='CodexRebuild.exe' OR Name='Codex.exe' OR Name='codex.exe'\" | "
        f"Where-Object {{ $_.ExecutablePath -like '{expected_root}\\*' }} | "
...
def launch_and_inject(app_dir: Path | None, db_path: Path | None, backup_dir: Path, debug_port: int, helper_port: int) -> tuple[HelperServer, Any]:
    ...
    try:
        ...
        running_codex_pid = running_windows_codex_process_id(resolved_app_dir) if can_reuse_windows_process else None
        if can_reuse_windows_process and _loopback_port_listening(debug_port) and running_codex_pid is not None:
            codex_proc = running_codex_pid
        else:
        ...
        # Clean up only the Rebuild process managed by this app_dir.
        if sys.platform == "win32":
            try:
                expected_root = str(resolved_app_dir).lower().replace("/", "\\")
                cleanup_script = (
                    "Get-CimInstance Win32_Process -Filter \"Name='CodexRebuild.exe'\" | "
                    f"Where-Object {{ $_.ExecutablePath -and $_.ExecutablePath.ToLower().Replace('/','\\') -like '{expected_root}\\*' }} | "
