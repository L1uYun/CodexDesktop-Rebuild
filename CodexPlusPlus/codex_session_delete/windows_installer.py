...
from codex_session_delete.rebuild_config import DEFAULT_APP_DIR, DEFAULT_DEBUG_PORT, SHORTCUT_NAME, UNINSTALL_DISPLAY_NAME, UNINSTALL_KEY_NAME
...
def _launcher_command(options: "InstallOptions") -> str:
    return options.launcher_command or f'{_default_python_executable()} -m codex_session_delete start --app-dir "{DEFAULT_APP_DIR}" --debug-port {DEFAULT_DEBUG_PORT}'
...
$ShortcutPath = Join-Path $InstallRoot '{SHORTCUT_NAME}'
...
$UninstallKey = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{UNINSTALL_KEY_NAME}'
...
Set-ItemProperty -Path $UninstallKey -Name DisplayName -Value '{UNINSTALL_DISPLAY_NAME}'
